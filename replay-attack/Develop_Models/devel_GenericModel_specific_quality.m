function devel_GenericModel_specific_quality(Feats_train_SL, Labels_train_SL,  Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, ...
    Feat_Type, method)
...
for i = 1:SUB_NUM_S  % we first assign for source subjects
    ...
    for b = 1:2
        ...
        samples_gen_data_sub = [samples_gen_data_sub; samples_data_subjects_genuine{s}(ind(1:int16(length(ind)/2)), :)];
    end
...
% ---------------- train SVM model ---------------- %
if exist(strcat('..\Train_Models\GenericModel_Sep_', name{Feat_Type}, '_', method, '_', suffix, '.mat'))
    load(strcat('..\Train_Models\GenericModel_Sep_', name{Feat_Type}, '_', method, '_', suffix, '.mat'));
    model = GenericModel;
...
for s = 1:SUB_NUM
    ...
    samples_data_train_fld_pos = samples_data_subjects_genuine{s}(1:3:end, :);
    samples_data_train_fld_neg = samples_data_subjects_fake{s}(1:3:end, :);
...
save(strcat('GenericPerf_Sep_devel_', name{Feat_Type}, '_', method, '_', suffix, '.mat'), 'Perf');
