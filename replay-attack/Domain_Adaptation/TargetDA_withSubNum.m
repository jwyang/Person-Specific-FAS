function TargetDA_withSubNum(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feats_enroll_SL, Labels_enroll_SL, Feat_Type, BLabel, method)
...
for s = 1:SUB_NUM
    ...
    samples_label_subjects_gen{s} = zeros(size(SubIDs_enroll));
end
...
for i = 1:length(SubIDs)
    ...
    s = s_rank(1);
    if strcmp(BLabels(i), BLabel) && strcmp(MLabels(i), 'FixN') % && strcmp(ALabels(i), 'IpN') && strcmp(MLabels(i), 'FixN') && strcmp(FLabels(i), 'PhN')
        samples_label_subjects{s}(i) = 1;
    elseif strcmp(BLabels(i), BLabel_gen) 
...
for i = 1:length(SubIDs_enroll)
    ...
    s = s_rank(1);
    if strcmp(BLabels_enroll(i), BLabel) % && strcmp(ALabels(i), 'IpN') && strcmp(MLabels(i), 'FixN') && strcmp(FLabels(i), 'PhN')
        samples_label_subjects_gen{s}(i) = 1;
    end
end
...
for i = 1:length(SubIDs)
    s_rank = find(SubIDs(i)==clientID);
    s = s_rank(1);
    if strcmp(BLabels(i), BLabel) && strcmp(MLabels(i), 'FixN')  % && strcmp(MLabels(i), 'FixN') && strcmp(FLabels(i), 'PhN')
        if Feat_Type == 1
...
for i = 1:length(SubIDs_enroll)
    s_rank = find(SubIDs_enroll(i)==clientID_enroll);
    s = s_rank(1);
    if strcmp(BLabels_enroll(i), BLabel)
        if Feat_Type == 1
...
% one subject domain
dist_thresholds = [0.1 0.2 0.2 0.2];
...
for SubNum = 1:SUB_NUM_S
    disp(strcat(num2str(SubNum), {' '}, 'source subjects are avaible'));
    for n_s = 1:SUB_NUM_T
        ...
        for m_s = 1:SubNum
            ...
            for p = 1:2
                for a = 1:3
                    for f = 1:2
                        ind = (strcmp(samples_mtype_subjects{m}, MTypes{p}) & strcmp(samples_atype_subjects{m}, ATypes{a}) & strcmp(samples_ftype_subjects{m}, FTypes{f}));
                        
                        samples_source = samples_data_subjects{m}(ind, :); %samples_data_subjects{m};
                        % L2-Normalize the features in source subject domain
                        ...
                        if ~strcmp(method, 'CS')  % if use OLS or PLS for adaptation, then translate the center
                            ...
                            H_mn_CS = Transform_CS.H{m, n};
                            T_mn_CS = Transform_CS.T{m, n};
                            synthdata = bsxfun(@plus, H_mn_CS*synthdata, T_mn_CS(:, 1));
                        else
                            H_mn = Transform.H{m, n};
                            T_mn = Transform.T{m, n};
                        ...
                        ind_valid = distmat_min >= dist_thresholds(Feat_Type);
                        SynthFeatures_TSub = [SynthFeatures_TSub; synthdata(:, ind_valid)'];
...
save(strcat('SynthFeatures_AllTSubs_withSubNum_Enroll_', BLabel, '_', name{Feat_Type}, '_', method), 'SynthFeatures_AllTSubs', '-v7.3');
