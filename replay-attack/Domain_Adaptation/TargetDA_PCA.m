function TargetDA_PCA(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feats_enroll_SL, Labels_enroll_SL, Feat_Type, BLabel, method)
    ...
    for a = 1:3
        for f = 1:2
            ...
            for i = 1:SUB_NUM_S
                ...
                ind = (strcmp(samples_mtype_subjects{s}, MTypes{p}) & strcmp(samples_atype_subjects{s}, ATypes{a}) & strcmp(samples_ftype_subjects{s}, FTypes{f}));
                if sum(ind) == 0
                    continue;
                end
                % ind = strcmp(samples_atype_subjects{s}, ATypes{a});
                
                samples_data_subject_fake = samples_data_subjects{s}(ind, :); %samples_data_subjects{m};
            ...
            if isempty(samples_data_allsubjects_matchings_fake_centralized)
                continue;
            end
            ...
            for m = 1:SUB_NUM_T
                samples_synthdata_subject_train_fake = bsxfun(@plus, Transform.weights{m}'*samples_data_allsubjects_matchings_fake_centralized, ...
                    samples_data_allsubject_matchings_fake_mean);
                SynthFeatures_AllTSubs{m} = [SynthFeatures_AllTSubs{m}; samples_synthdata_subject_train_fake];
            end
        end
    end
end

save(strcat('SynthFeatures_AllTSubs_Enroll_', BLabel, '_', name{Feat_Type}, '_', method), 'SynthFeatures_AllTSubs', '-v7.3');
...
samples_data_matched_fake = samples_data_subject_fake(pairs_matched(:, 2), :);
