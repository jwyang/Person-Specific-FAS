function train_PSModels(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type, method)
...
for b = 1:2
    VirtualFeatures{b} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_', BTypes_fake{b}, '_', name{Feat_Type}, '_', method));
end
...
for i = 1:SUB_NUM_T
    ...
    samples_data = VirtualFeatures{1}.SynthFeatures_AllTSubs{i};
    samples_data = [samples_data; VirtualFeatures{2}.SynthFeatures_AllTSubs{i}];
...
save(strcat('PSModels_', name{Feat_Type}, '_', method, '.mat'), 'PSModels');
end
