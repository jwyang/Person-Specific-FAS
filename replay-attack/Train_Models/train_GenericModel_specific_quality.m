function train_GenericModel_specific_quality(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, ...
...
for i = 1:length(SubIDs)
    ...
    elseif strcmp(MLabels(i), Quality_Type_Fake) && sum(SubIDs(i) == clientID_source) > 0
        if Feat_Type == 1
            samples_data_subjects_fake{s}(sample_sub_id_fake(s), :) = Feats{i}.MsLBP{1};
        elseif Feat_Type == 2
            samples_data_subjects_fake{s}(sample_sub_id_fake(s), :) = Feats{i}.LBP{1};
...
if ~strcmp(method, 'NULL')
    VirtualFeatures = cell(length(QualityType), 1);    
    for q = 1:length(VirtualFeatures)
         VirtualFeatures{q} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_', QualityType{q}, '_', name{Feat_Type}, '_', method));
    end
end
...
if ~strcmp(method, 'NULL')
    for s = 1:SUB_NUM_T
        samples_data = [];
        for q = 1:length(VirtualFeatures)
            samples_data = [samples_data; VirtualFeatures{q}.SynthFeatures_AllTSubs{s}];
        end
...
% ---------------- train SVM model ---------------- %
% if exist(strcat('GenericModel_', name{Feat_Type}, '_', suffix, '.mat'))
%     load(strcat('GenericModel_', name{Feat_Type}, '_', suffix, '.mat'));
%     model = GenericModel;
% else
