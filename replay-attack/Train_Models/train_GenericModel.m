function train_GenericModel(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type)
...
for i = 1:length(SubIDs)
    ...
    elseif strcmp(MLabels(i), 'FixN') && sum(SubIDs(i) == clientID_source) > 0   % if the sample is from source subject, then add it into training set
...
for i = 1:length(SubIDs)
    ...
    elseif strcmp(MLabels(i), 'FixN') && sum(SubIDs(i) == clientID_source) > 0
        if Feat_Type == 1
...
if ~exist(strcat('GenericModel_', name{Feat_Type}, '.mat'))
    GenericModel = model;
    save(strcat('GenericModel_', name{Feat_Type}, '.mat'), 'GenericModel');
end
