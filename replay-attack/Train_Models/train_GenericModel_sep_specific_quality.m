function train_GenericModel_sep_specific_quality(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, ...
...
if ~strcmp(method, 'NULL')
    ...
    for q = 1:length(QualityType)
         VirtualFeatures{q, b} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_', QualityType{q}, '_', BType{b}, '_', name{Feat_Type}, '_', method));
    end
    end
end
...
if ~strcmp(method, 'NULL')
    for s = 1:SUB_NUM_T
        ...
        for b = 1:length(BType)
        for q = 1:length(QualityType)
            samples_data = [samples_data; VirtualFeatures{q, b}.SynthFeatures_AllTSubs{s}];
        end
        end
...
% if ~exist(strcat('GenericModel_', name{Feat_Type}, '_', method, '_', suffix, '.mat'))
    GenericModel = model;
    save(strcat('GenericModel_Sep_', name{Feat_Type}, '_', method, '_', suffix, '.mat'), 'GenericModel');
% end
