function train_GenericModel_withSubNum_specific_quality(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, ...
...
for SubNum = 1:SUB_NUM_S
    ...
    for s_u = 1:SUB_NUM_T
        ...
        if ~strcmp(method, 'NULL')   % add synthesized fake samples for training generic models 
            for b = 1:length(BType)
                for q = 1:length(QualityType)
                    samples_data_train_fld_neg = [samples_data_train_fld_neg; VirtualFeatures{q, b}.SynthFeatures_AllTSubs{s_u, SubNum}];
                end
            end
        end
    end
    
    % add synthesized features if needed
    
    samples_data_train_fld_pos = samples_data_train_fld_pos(1:5:end, :);
    samples_data_train_fld_neg = samples_data_train_fld_neg(1:5:end, :);
    ...
    svm_param = '-t 0 -c 10 -h 0';
    
    % ---------------- train SVM model ---------------- %    
    GenericModel{SubNum} = svmtrain(samples_label_fld_train, samples_data_fld_train, svm_param);
...
save(strcat('GenericModel_withSubNum_', name{Feat_Type}, '_', method, '_', suffix, '.mat'), 'GenericModel');
