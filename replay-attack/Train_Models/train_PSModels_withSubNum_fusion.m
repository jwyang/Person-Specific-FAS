function train_PSModels_withSubNum_fusion(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type, method)
...
for b = 1:2
    for k = 1:length(Feat_Type)
        VirtualFeatures{b, k} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_withSubNum_', BTypes_fake{b}, '_', name{Feat_Type(k)}, '_', method));
    end
end
...
for SubNum = 1:SUB_NUM_S
    for i = 1:SUB_NUM_T
        ...
        samples_data_q1 = [];
        for k = 1:length(Feat_Type)
            data = VirtualFeatures{1, k}.SynthFeatures_AllTSubs{i, SubNum};
            if isempty(data)
                break;
            end
        ...
        samples_data_q2 = [];
        for k = 1:length(Feat_Type)
            data = VirtualFeatures{2, k}.SynthFeatures_AllTSubs{i, SubNum};
            if isempty(data)
                break;
            end
            data = bsxfun(@rdivide, data, sqrt(sum(data.^2, 2)));
