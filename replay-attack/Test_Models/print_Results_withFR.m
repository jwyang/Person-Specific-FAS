...
for t = 1:length(Feat_Names)
    for m = 1:length(Methods_PS)
        Perfs_PS{t, m} = load(strcat('PerSpecPerf_Sep_crosstest_', Feat_Names{t}, '_', Methods_PS{m}, '_Enroll.mat'));
    end
end
...
for t = 1:length(Feat_Names)
    ...
    for m = 1:length(Methods_PS)
        error = lfr_process(Perfs_PS{t, m}.Perf, [fr_results_subjects_genuine; fr_results_subjects_fake], ind_S, clientID);
    ...
    for m = 1:length(Methods_PS)
        ...
        error = lfr_process(Perfs_PS{t, m}.Perf, [fr_results_subjects_genuine; fr_results_subjects_fake], ind_T, clientID);
        error_disp = sprintf('%3.2f', 100 * error);        
        disp_str = strcat(disp_str, '&', {' '}, error_disp, {' '});
    end
    display(disp_str);
    ...
    for m = 1:length(Methods_PS)
        ...
        error = lfr_process(Perfs_PS{t, m}.Perf, [fr_results_subjects_genuine; fr_results_subjects_fake], [1:50], clientID);
        error_disp = sprintf('%3.2f', 100 * error);        
        disp_str = strcat(disp_str, '&', {' '}, error_disp, {' '});
    end
    display(disp_str);
