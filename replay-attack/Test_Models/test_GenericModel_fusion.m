function test_GenericModel_fusion(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type)
...
% ---------------- train SVM model ---------------- %
if exist(strcat('..\Train_Models\GenericModel', modelname, '.mat'))
    load(strcat('..\Train_Models\GenericModel', modelname, '.mat'));
    model = GenericModel;
...
load(strcat('..\Develop_Models\GenericPerf_devel', modelname, '.mat'));
...
FAR = sum(Scores_fake >= Perf.Threshold)/(length(Scores_fake));
FRR = sum(Scores_gen < Perf.Threshold)/(length(Scores_gen));
HTER = (FAR + FRR)/2;    
...
for s = 1:SUB_NUM
    FAR = sum(Scores_subs_fake{s} >= Perf.Threshold)/(length(Scores_subs_fake{s}));
    FRR = sum(Scores_subs_gen{s} < Perf.Threshold)/(length(Scores_subs_gen{s}));
    HTERs(s) = (FAR + FRR)/2;    
end
...
save(strcat('GenericPerf_test', modelname, '.mat'), 'Perf');
