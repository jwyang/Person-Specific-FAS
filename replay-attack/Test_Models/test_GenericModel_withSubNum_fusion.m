function test_GenericModel_withSubNum_fusion(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type)
...
% ---------------- train SVM model ---------------- %
if exist(strcat('..\Train_Models\GenericModel_withSubNum', modelname, '.mat'))
    load(strcat('..\Train_Models\GenericModel_withSubNum', modelname, '.mat'));
...
load(strcat('..\Develop_Models\GenericPerf_devel_withSubNum', modelname, '.mat'));
...
for SubNum = 1:SUB_NUM_S
    ...
    FAR = sum(Scores_fake >= Perf{SubNum}.Threshold)/(length(Scores_fake));
    FRR = sum(Scores_gen < Perf{SubNum}.Threshold)/(length(Scores_gen));
    HTER = (FAR + FRR)/2;
    ...
    for s = 1:SUB_NUM
        FAR = sum(Scores_subs_fake{s} >= Perf{SubNum}.Threshold)/(length(Scores_subs_fake{s}));
        FRR = sum(Scores_subs_gen{s} < Perf{SubNum}.Threshold)/(length(Scores_subs_gen{s}));
        HTERs(s) = (FAR + FRR)/2;
    end
...
save(strcat('GenericPerf_test_withSubNum', modelname, '.mat'), 'Perf');
