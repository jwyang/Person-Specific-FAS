function test_FASLFRModels_specific_quality(Feats_train_SL, Labels_train_SL, Feats_devel_SL, Labels_devel_SL, Feats_test_SL, Labels_test_SL, Feat_Type, method)
...
load(strcat('..\Train_Models\PSModels_Sep', modelname, '_', method, '_', suffix, '.mat'));
load(strcat('..\Train_Models\GenericModel', modelname, '_NULL_', suffix, '.mat'));

% Load thrsholds for computing HTER
load(strcat('..\Develop_Models\FASPerf_devel_Sep', modelname, '_', method, '_', suffix, '.mat'));
...
for s = 1:SUB_NUM
    ...
    [~, ~, scores_generic{s}] = svmpredict(samples_label_fld_train,samples_data_fld_train, GenericModel);
end
...
for s = 1:SUB_NUM
    ...
    % L2 Normalize feature vectors, has normalized before
    samples_data_fld_train = bsxfun(@rdivide, samples_data_fld_train, sqrt(sum(samples_data_fld_train.^2, 2)));

    for c = 1:SUB_NUM
        % ------------- test trained SVM model by trainning data --------------- %
        [~,~,scores_specific] = svmpredict(samples_label_fld_train,samples_data_fld_train, PSModels{c});
        scores = scores_specific + scores_generic{s};
...
Perf.clientID_source = clientID_source;
Perf.clientID_target = clientID_target;
Perf.clientID = clientID;

save(strcat('FASPerf_Sep_crosstest', modelname, '_', method, '_', suffix, '.mat'), 'Perf');
