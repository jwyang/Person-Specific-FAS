function test_GenericModel_withSubNum_fusion_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type_train, Quality_Type_test, method)
...
% L2 Normalize feature vectors, has normalized both MsLBP and HOG before
...
if exist(strcat('..\Train_Models\GenericModel_withSubNum', modelname, '_', method, '_', suffix, '.mat'))
    load(strcat('..\Train_Models\GenericModel_withSubNum', modelname, '_', method, '_', suffix, '.mat'));
    model = GenericModel;
...
load(strcat('..\Develop_Models\GenericPerf_devel_withSubNum', modelname, '_', method, '_', suffix, '_', Quality_Type_test, '.mat'));
...
for SubNum = 1:SUB_NUM_S
    ...
    FAR = sum(Scores_fake >= Perf{SubNum}.Threshold_overall)/(length(Scores_fake));
    FRR = sum(Scores_gen < Perf{SubNum}.Threshold_overall)/(length(Scores_gen));
    HTER = (FAR + FRR)/2;
    ...
    for s = 1:SUB_NUM
        FAR = sum(Scores_subs_fake{s} >= Perf{SubNum}.Threshold_overall)/(length(Scores_subs_fake{s}));
        FRR = sum(Scores_subs_gen{s} < Perf{SubNum}.Threshold_overall)/(length(Scores_subs_gen{s}));
        HTERs(s) = (FAR + FRR)/2;
    end
    ...
    Perf{SubNum}.EER_S = EER_S;
    Perf{SubNum}.EER_T = EER_T;
    Perf{SubNum}.EER = EER;
    Perf{SubNum}.HTERs = HTERs;
    Perf{SubNum}.HTER = HTER;
    Perf{SubNum}.Scores_gen = Scores_subs_gen;
    Perf{SubNum}.Scores_fake = Scores_subs_fake;
end

save(strcat('GenericPerf_test_withSubNum',modelname, '_', method, '_', suffix, '_', Quality_Type_test, '.mat'), 'Perf');
