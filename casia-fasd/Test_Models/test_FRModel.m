function test_FRModel( Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type)
...
load(strcat('..\Train_Models\FRModel_small_', name{Feat_Type}, '.mat'));
...
for s = SUB_NUM_S + 1:SUB_NUM
    ...
    DistMatrix = zeros(size(transform_samples_data_fr_test, 2), 50);
    
    for k = 1:SUB_NUM
        DistMatrix(:, k) = mean(pdist2(transform_samples_data_fr_test', train_samples_transform{k}'), 2);
    end
...
save(strcat('FRPerf_test_small_', name{Feat_Type}, '.mat'), 'FRLabels');
