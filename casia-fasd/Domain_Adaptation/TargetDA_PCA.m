function TargetDA_PCA(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, QualityType, method)
...
samples_patches_allsubs = cell(SUB_NUM_S, 1);
...
for s = 1:SUB_NUM_S
    ...
    if ratio ~= 0
        ...
        for r = 1:ratio
            ...
            samples_patches_allsubs{s}{r} = findMatchings(samples_data_subject_genuine, samples_patch);
        end
    else
        ratio = 1;
        samples_patches_allsubs{s} = cell(ratio, 1);
...
for r = 1:ratio_mean
    ...
    for i = 1:SUB_NUM_S
        if ratios(i) >= r
            samples_data_subject_matchings_fake{i} = samples_patches_allsubs{i}{r};
        else
            samples_data_subject_matchings_fake{i} = samples_patches_allsubs{i}{ratios(i)};
        end
    ...
    for m = SUB_NUM_S+1:SUB_NUM
        samples_synthdata_subject_train_fake = bsxfun(@plus, Transform.weights{m-SUB_NUM_S}'*samples_data_allsubjects_matchings_fake_centralized, ...
            samples_data_allsubject_matchings_fake_mean);
        SynthFeatures_AllTSubs{m-SUB_NUM_S} = [SynthFeatures_AllTSubs{m-SUB_NUM_S}; samples_synthdata_subject_train_fake];
    end
end

save(strcat('SynthFeatures_AllTSubs_', QualityType, '_', name{Feat_Type}, '_', method), 'SynthFeatures_AllTSubs', '-v7.3');
