function train_FRModel(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type)
...
for q = 1:3
    VirtualFeatures{q} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_', QualityTypes_Fake{q}, '_', name{Feat_Type}, '_', 'CS'));
end
...
for s = 1:SUB_NUM
    % reassign genuine samples
    samples_data_sub = [samples_data_subjects_genuine{s}]; % ; samples_data_subjects_fake{s}
...
save(strcat('FRModel_small_',  modelname, '.mat'), 'FRModel');
