function train_PSModels_fusion_withSubNum_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type, method)
...
for q = 1:length(QualityType_Fake)
    for k = 1:length(Feat_Type)
        VirtualFeatures{q, k} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_withSubNum_', QualityType_Fake{q}, '_', name{Feat_Type(k)}, '_', method));
    end
end
...
for SubNum = 1:SUB_NUM_S    
    for s = SUB_NUM_S+1:SUB_NUM        
        ...
        for q = 1:length(QualityType_Fake)
            samples_data_feat = [];
            for k = 1:length(Feat_Type)
                data = VirtualFeatures{q, k}.SynthFeatures_AllTSubs{s-SUB_NUM_S, SubNum};
                if isempty(data)
                    break;
                end
...
save(strcat('PSModels_withSubNum', modelname, '_', method, '_', suffix, '.mat'), 'PSModels', '-v7.3');
end
