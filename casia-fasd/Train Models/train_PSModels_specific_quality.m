function train_PSModels_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type, method)
...
for q = 1:length(VirtualFeatures)
    VirtualFeatures{q} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_noOutlier_', QualityType_Fake{q}, '_', name{Feat_Type}, '_', method));
end
...
save(strcat('PSModels_noOutlier_', name{Feat_Type}, '_', method, '_', suffix, '.mat'), 'PSModels');
end
...
for t = 0
    for c = 100
        svm_param = '-t 0 -c 10 -g 0.1'; % strcat('-t ',num2str(t), ' -c ', num2str(c), ' -g 0.5 -h 0');
