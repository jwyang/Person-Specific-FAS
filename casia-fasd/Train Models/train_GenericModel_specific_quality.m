function train_GenericModel_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type, method)
%TRAIN_GENERICMODEL_SPECIFIC_QUALITY 此处显示有关此函数的摘要
%   此处显示详细说明
...
for i = 1:length(SubIDs)
    s = SubIDs(i);
    if s <= SUB_NUM_S
        if sum(strcmp(QualityType_Gen, QLabels(i))) > 0
            samples_label_subjects_genuine{s}(i) = 1;
        elseif sum(strcmp(QualityType_Fake, QLabels(i))) > 0
...
if ~strcmp(method, 'NULL')
    ...
    for q = 1:length(VirtualFeatures)
        ...
        VirtualFeatures{q} = load(strcat('..\Domain_Adaptation\SynthFeatures_AllTSubs_', QualityType_Fake{q}, '_', name{Feat_Type}, '_', method));
    end
end
...
for s = 1:SUB_NUM
    ...
    if s > SUB_NUM_S && ~strcmp(method, 'NULL')
        samples_data = [];
        for q = 1:length(VirtualFeatures)
            samples_data = [samples_data; VirtualFeatures{q}.SynthFeatures_AllTSubs{s-SUB_NUM_S}];
        end
        samples_data_train_fld_neg = [samples_data_train_fld_neg; samples_data];
    end
end
...
for t = 0
    for c = 100
        svm_param = '-t 0 -c 10 -g 5'; % strcat('-t ',num2str(t), ' -c ', num2str(c), ' -g 0.5 -h 0');
        
        % ---------------- train SVM model ---------------- %
        if exist(strcat('GenericModel_', name{Feat_Type}, '.mat'))
            load(strcat('GenericModel_', name{Feat_Type}, '.mat'));
            model = GenericModel;
            break;
        else
...
if ~exist(strcat('GenericModel_', name{Feat_Type},  '_', method, '_', suffix, '.mat'))
    GenericModel = model;
    save(strcat('GenericModel_', name{Feat_Type},  '_', method, '_', suffix, '.mat'), 'GenericModel');
end
