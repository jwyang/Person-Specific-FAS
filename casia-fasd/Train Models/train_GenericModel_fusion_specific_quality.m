function train_GenericModel_fusion_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type, method)
    ...
    if s > SUB_NUM_S && ~strcmp(method, 'NULL')
        ...
        for q = 1:size(VirtualFeatures, 1)
            ...
            for k = 1:size(VirtualFeatures, 2)
                data = VirtualFeatures{q, k}.SynthFeatures_AllTSubs{s-SUB_NUM_S};
                if isempty(data)
                    break;
                end
...
if exist(strcat('GenericModel_', name{Feat_Type}, '_', method, '_', suffix, '.mat'))
    load(strcat('GenericModel_', name{Feat_Type}, '_', method, '_', suffix, '.mat'));
    model = GenericModel;
else
...
modelname = strcat(modelname, '_', method, '_', suffix, '.mat');

if ~exist(modelname)
    GenericModel = model;
    save(modelname, 'GenericModel');
end
