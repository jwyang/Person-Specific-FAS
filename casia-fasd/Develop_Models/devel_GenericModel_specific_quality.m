function devel_GenericModel_specific_quality(Feats_train_SL, Labels_train_SL, Feats_test_SL, Labels_test_SL, Feat_Type, Quality_Type_train, Quality_Type_dev, method)
...
%{
samples_data_devel_fld_pos = [];
samples_data_devel_fld_neg = [];

for s = 1:SUB_NUM
    samples_data_devel_fld_pos = [samples_data_devel_fld_pos; samples_data_subjects_genuine{s}];
    samples_data_devel_fld_neg = [samples_data_devel_fld_neg; samples_data_subjects_fake{s}];
end

samples_data_fld_train  =  [samples_data_devel_fld_pos; samples_data_devel_fld_neg];
samples_label_fld_train =  [ones(size(samples_data_devel_fld_pos, 1), 1); -1*ones(size(samples_data_devel_fld_neg, 1), 1)];
...
disp(strcat('meanEER: ', num2str(mean(EERs))));
...
Perf.EER_overall = EER;
Perf.Threshold_overall = threshold;

Perf.HTERs_persub  = HTERs;
Perf.EERs_persub = EERs;
Perf.Thresholds_persub    = thresholds;

save(strcat('GenericPerf_devel_', name{Feat_Type}, '_', method, '_', suffix, '_', Quality_Type_dev, '.mat'), 'Perf');
